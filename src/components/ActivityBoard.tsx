  ...
  return (
    <section id="activities" className="py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6 bg-white p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-100">
          
          {/* Segmented Control Tab Bar */}
          <div className="flex w-full lg:max-w-4xl overflow-x-auto whitespace-nowrap bg-slate-100/90 p-1.5 rounded-2xl border border-slate-200/50 shadow-inner gap-1 scrollbar-none">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  type="button"
                  className={`inline-flex items-center justify-center gap-1 px-2.5 py-2 sm:px-3.5 sm:py-2.5 lg:px-5 lg:py-3 text-[11px] sm:text-xs md:text-sm lg:text-base font-extrabold rounded-xl transition-all duration-200 cursor-pointer shrink-0 group ${
                    isActive
                      ? 'bg-[#1E3A8A] text-white shadow-md'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'
                  }`}
                >
                  <Icon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 transition-transform duration-200 ${isActive ? 'scale-110 text-white' : 'text-slate-400 group-hover:text-slate-600'}`} />
                  <span>{tab.label}</span>
                </button>
              );
            })}
